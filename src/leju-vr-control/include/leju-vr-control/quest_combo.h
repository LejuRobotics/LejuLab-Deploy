...
/// Y：左次（特殊状态判定，非退出程序）
inline bool isYPressed(const QuestJoystickData& joy) {
  return joy.left_second_button_pressed;
}

// X+Y 触摸判定：左主 + 左次（特殊状态判定，非退出程序）
inline bool isXYTouched(const QuestJoystickData& joy) {
  return joy.left_first_button_touched && joy.left_second_button_touched;
}
