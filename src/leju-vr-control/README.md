...
`leju-vr-control` 是面向使用者的 Quest3 VR 控制节点。它负责将 Quest3 的输入转换为机器人控制行为，包括手臂增量控制、头部控制、速度指令，以及 `Kuavo5(52)` 的腰部手柄控制。
...
- `Kuavo4Pro(45/46)` 不支持腰部摇杆控制
- `Roban(14)` 当前不支持该节点
- `Kuavo5(52)` 的腰部控制与 walking 控制互斥
