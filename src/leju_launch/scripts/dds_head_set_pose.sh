#!/usr/bin/env bash
# dds_head_set_pose.sh — Set head yaw/pitch via DDS /rt/head_trajectory
#
# Usage:
#   bash src/leju_launch/scripts/dds_head_set_pose.sh --depth-walk
#   bash src/leju_launch/scripts/dds_head_set_pose.sh --pitch-deg 45
#   bash src/leju_launch/scripts/dds_head_set_pose.sh --pitch-deg 30 --yaw-deg 0 --hold-duration 10
#
# Requires: pip3 install cyclonedds==0.10.2
# Run while launch_real.sh / rl_controller is active.
#
# Roban S17 /rt/head_trajectory convention:
#   q[0] = head_yaw (rad), q[1] = head_pitch (rad, positive = look down)
...
exec python3 "${SCRIPT_DIR}/dds_head_set_pose.py" "$@"
