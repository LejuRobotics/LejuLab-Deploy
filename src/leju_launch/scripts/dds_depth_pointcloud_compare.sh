#!/usr/bin/env bash
# dds_depth_pointcloud_compare.sh — Raw vs policy depth as point cloud comparison video
#
# Usage:
#   bash src/leju_launch/scripts/dds_depth_pointcloud_compare.sh
#   bash src/leju_launch/scripts/dds_depth_pointcloud_compare.sh -o docs/depth_pcd.mp4 --duration 15 --no-show
#   bash src/leju_launch/scripts/dds_depth_pointcloud_compare.sh --view both --raw-step 2
#
...
exec python3 "${SCRIPT_DIR}/dds_depth_pointcloud_compare.py" "$@"
