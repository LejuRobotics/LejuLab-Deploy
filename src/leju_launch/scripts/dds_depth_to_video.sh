#!/usr/bin/env bash
# dds_depth_to_video.sh — Preview / record depth camera DDS topic as MP4
#
# Usage:
#   bash src/leju_launch/scripts/dds_depth_to_video.sh
#   bash src/leju_launch/scripts/dds_depth_to_video.sh -o /tmp/depth.mp4 --duration 10
#   bash src/leju_launch/scripts/dds_depth_to_video.sh --match-controller --no-show -o depth.mp4
#
...
exec python3 "${SCRIPT_DIR}/dds_depth_to_video.py" "$@"
