...
| `TransportFallStand` | 搬运/倒地起身调度事件（手柄专用） | `name`: 事件名（见下） |
...
### TransportFallStand — 搬运 / 倒地起身（手柄）

一次性调度事件，经 `TriggerBuffer` → `ControlLogic` 交给进程内 `TransportFallStandScheduler`。  
**不是** DDS 运控命令；运控原语由 Scheduler 再调用 coordinator。

```yaml
- buttons: ["LB", "RB", "Y"]
  action:
    type: TransportFallStand
    args:
      name: transport.enter
```

| `args.name` | 语义 |
|-------------|------|
| `transport.enter` | 进入搬运 |
| `transport.lock` | 锁定可搬 |
| `transport.exit` | 退出/取消（调度层内部 `HAND_OVER`） |
| `transport.fall_down` | 软倒地 |
| `fallstand.standup` | 起身（按当前搬运状态分硬/软/独立链） |

可选字段：`hold`（秒）— 长按才触发（如 fall_down 2s）。

状态机、门控与退出时序见 [transport_fall_stand.md](transport_fall_stand.md)。
...
### Roban 2.2（ROBOT_VERSION=17）— 搬运 / 倒地起身
...
> 模块仅在 Roban 2.2 组装层注入；其它版本配置了绑定也不会生效。
...
- **`hold`**：配置了 `hold` 时需持续按住达到时长才触发一次（如 fall_down）
...
- 需同时按住所有按键才能匹配
- 扳机键（`LT`/`RT`）按 `velocity_limits.trigger_threshold` 判定按下
