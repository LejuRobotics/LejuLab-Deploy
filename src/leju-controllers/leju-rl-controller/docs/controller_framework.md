...
|  (Roban2.2) Transport/FallStand coordinators + joy scheduler          |
+-----------------------------------+-----------------------------------+
                                    |
                 +------------------+------------------+
                 |                                     |
    ...
    +---+------------------+----+        |  4. coordinators+scheduler |
        |                  |             |  5. ControlLogic.tick()    |
        v                  v             |  6. ControllerManager.update|
  +-----------+    +-------------+       |  7. publishRobotCmd()      |
  ...
  +-----------+    +-------------+                      |
       ...
       | - Auto Transition |
       | - Fall Logic      |
       | - TransportFallStand → Scheduler (joy) |
       +--------+----------+
                |
       +--------v-------------------+
       |    ControllerManager       |
...
3. 更新 `Lifecycle` 状态机
4. （Roban 2.2）`TransportModeCoordinator` / `FallStandCoordinator` / `TransportFallStandScheduler` tick  
   （非 Running 也会跑：状态发布 / 边沿与退出编排推进；命令副作用 internally 受 `running` 门控）
5. 若非 Running，直接返回
6. 按优先级合并所有 `InputSource` 的 `CommandBuffer` 快照
7. `ControlLogic.tick()` — 策略决策（含手柄 `TransportFallStand` → `scheduler.onEvent`）
8. `ControllerManager.update()` — 执行当前活跃控制器
9. 发布 `RobotCmd` 到机器人
...
2. `handleActionTriggers()` — 处理离散事件（SwitchController、SetArmMode、`TransportFallStand` 等）
3. `handleAutoTransitions()` — 控制器完成后自动切换（如 GroundToStand → AMP）
4. `handleFallLogic()` — 检测跌倒并触发保护控制（ACTIVE 搬运时可由 coordinator 豁免）

**触发去重策略：**
- 敏感类型（Start、Quit、StartMotion）：每 tick 仅保留一个
- LastWins 类型（SwitchController、SetArmMode）：保留最后一个

**搬运门控（可选注入 `TransportModeCoordinator`）：** 非 INACTIVE 时拒绝 SwitchController / SetArmMode / MotionCommand 等；`TransportFallStand` 不受门控，交给 `TransportFallStandScheduler`。详见 [transport_fall_stand.md](transport_fall_stand.md)。
...
enum class ActionType : uint8_t {
  ...
  TransportFallStand,  // 手柄搬运/倒地起身调度事件
};
...
| 5 | 搬运/倒地起身 | TransportFallStand → Scheduler |
...
- Roban 2.2 另有搬运 ACTIVE 期间的跌倒保护豁免，以及 `mimic_fall_stand` 细粒度起身通道（见下节）

## 搬运模式与倒地起身（Roban 2.2）

三层分工：

1. **运控原语** — `TransportModeCoordinator` / `FallStandCoordinator`：DDS 对外 + 进程内 `sendCommand`；一条命令原子副作用，零编排
2. **手柄调度** — `TransportFallStandScheduler`：仅 joy 路径，展开多步时序（保护窗、硬/软/独立起身链、退出 amp+arm+EXIT）
3. **ControlLogic** — 门控查询 + Phase 5 转发事件

外部上位机直接打 `/rt/transport_mode_command`、`/rt/fall_stand_command`，**不经** Scheduler。

完整状态机、按键、退出时序见 [transport_fall_stand.md](transport_fall_stand.md)。
