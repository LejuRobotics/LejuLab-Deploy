...
TEST(ActionTypeTest, EnumValuesAreUnique) {
  ...
  EXPECT_NE(ActionType::None, ActionType::MotionCommand);
}
...
TEST(ActionTypeTest, EnumToString) {
  ...
  EXPECT_EQ(ActionTypeToString(ActionType::MotionCommand), "MotionCommand");
}
...
TEST(ParseActionTypeTest, ValidTypesPascalCase) {
  ...
  EXPECT_EQ(ParseActionType("MotionCommand"), ActionType::MotionCommand);
}
...
joy_bindings:
  ...
  ComboKey key6;
  key6.buttons = {"RB", "B"};
  EXPECT_EQ(joy_config.findBinding(key6)->action.type, ActionType::MotionCommand);

  std::remove(path.c_str());
}

TEST(TeleopBindingConfigTest, LoadFromFile_EmptyConfig) {
  // 空配置缺少必须的 velocity_limits 段，loadFromFile 应返回 false
  std::string yaml = "";
  std::string path = CreateTempYamlFile(yaml);
  TeleopBindingConfig config;
  EXPECT_FALSE(config.loadFromFile(path));
