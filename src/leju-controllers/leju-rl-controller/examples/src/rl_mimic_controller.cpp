#include "leju-rl-controller/examples/rl_mimic_controller.h"
...
bool RLMimicConfig::loadFromYaml(YAML::Node root_node) {
  ...
  // 从 motion.motions 中根据 default_motion 查找对应的 file
  std::string default_motion = humanoid_cfg["motion"]["default_motion"].as<std::string>();
  YAML::Node motions = humanoid_cfg["motion"]["motions"];
  for (const auto& motion : motions) {
    if (motion["name"].as<std::string>() == default_motion) {
      motion_data_path = motion["file"].as<std::string>();
      break;
    }
  }
