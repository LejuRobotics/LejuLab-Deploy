...
void QuestTeleopAdapter::onQuestData(const QuestJoystickData& data) {
  ...
  // 整帧持锁: 与热重载 (setBindingConfig) 互斥, 保护下游对 config_ / binding_config_ 的读.
...
double QuestTeleopAdapter::getCmdVelLineXLimitForLevel(int level) const {
...
double QuestTeleopAdapter::getEffectiveMaxLinearXForward() const {
  const auto& gear_cfg = config_.amp_hand_cmd_vel_line_x_gear;
...
double QuestTeleopAdapter::getEffectiveMaxLinearXBackward() const {
  const auto& gear_cfg = config_.amp_hand_cmd_vel_line_x_gear;
  if (gear_cfg.enabled) {
...
void QuestTeleopAdapter::processVelocityImpl(const QuestJoystickData& data,
  ...
  // A+B 触摸门控
  const bool ab_touched = data.right_first_button_touched &&
                          data.right_second_button_touched;

  MotionCommand cmd;
  cmd.linear_x = left_y;
  cmd.linear_x *= cmd.linear_x > 0 ? getEffectiveMaxLinearXForward() : getEffectiveMaxLinearXBackward();
  cmd.linear_y = ab_touched ? (-left_x * config_.max_linear_y) : 0.0f;
