...
void CommandBuffer::writeHandTarget(const ExternalHandTarget& target) {
  size_t current = active_index_.load(std::memory_order_relaxed);
  size_t write_index = 1 - current;

  buffers_[write_index] = buffers_[current];
  buffers_[write_index].hand_target.value = target;
  buffers_[write_index].hand_target.seq = buffers_[current].hand_target.seq + 1;
  active_index_.store(write_index, std::memory_order_release);
}
...
void CommandBuffer::clearCmdVel() {
  size_t current = active_index_.load(std::memory_order_relaxed);
  size_t write_index = 1 - current;

  buffers_[write_index] = buffers_[current];
  buffers_[write_index].cmd_vel.setZero();
  buffers_[write_index].cmd_vel.valid = true;
  active_index_.store(write_index, std::memory_order_release);
}
