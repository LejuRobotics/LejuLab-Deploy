#include "leju-rl-controller/rl_mimic_controller.h"
...
void RLMimicController::computeActions() {
  ...
  // std::cout << "[DEBUG] [RLMimicController::computeActions] after infer" <<
  // std::endl;
...
void RLMimicController::updateRobotCmd() {
  ...
  for (int i = 0; i < policy_joint_count_; i++) {
    cmd_.q[policy_joint_ids_[i]] =
        cfg_.control_mode[i] == 2 ? policy_q_target[i] : policy_q[i];
    cmd_.v[policy_joint_ids_[i]] = 0.;
    // cmd_.tau[policy_joint_ids_[i]] = policy_torque_demand[i];
    if (cfg_.control_mode[i] == 2) {
      cmd_.tau[policy_joint_ids_[i]] = 0.;
    } else if (cfg_.control_mode[i] == 1) {
      cmd_.tau[policy_joint_ids_[i]] =
          cfg_.kp[i] * (policy_q_target[i] - policy_q[i]);
    } else {
      cmd_.tau[policy_joint_ids_[i]] =
          cfg_.kp[i] * (policy_q_target[i] - policy_q[i]) +
          cfg_.kd[i] * (-policy_v[i]);
    }
    cmd_.modes[policy_joint_ids_[i]] = cfg_.control_mode[i] == 0 ? 0 : 2;
    if (cfg_.control_mode[i] == 0) {
      ...
      // 设 kp=kd=0 避免双重阻尼
      /////////////////////////////////////////////////
      cmd_.kp[policy_joint_ids_[i]] = 0.;
      cmd_.kd[policy_joint_ids_[i]] = 0.;
    } else {
  ...
  // std::cout << "[DEBUG] [RLMimicController::updateRobotCmd] "
