...
void ControllerManager::stop() {
  ...
  RobotState state;
  int motor_count = robot_data_.getRobotState(state) ? static_cast<int>(state.q.size()) : 0;
  if (motor_count > 0) {
    for (int i = 0; i < 50; ++i) {
...
void ControllerManager::update() {
  ...
  RobotCmd cmd;
  if (controllers_[active_index_].controller->update(time, state, imu, cmd)) {
  ...
  try {
    ...
    for (const auto& ctrl_node : controllers_node) {
      ...
      std::string ctrl_config_path =
          (std::filesystem::path(config_dir_) / ctrl_config).string();
      ...
      if (!controller->initialize()) {
        RL_LOG_FAILURE("Failed to initialize controller: %s", name.c_str());
        continue;
      }
