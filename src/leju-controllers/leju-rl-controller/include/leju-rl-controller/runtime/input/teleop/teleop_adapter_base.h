...
public:
  ...
  void clearCmdVel() override { cmd_buffer_.clearCmdVel(); }
  ...
  bool loadBindingConfig(const std::string& config_path) {
    TeleopBindingConfig loaded;
    if (!loaded.loadFromFile(config_path)) {
      return false;
    }
    // 仅在文件解析成功后才上锁替换, 避免半加载状态被 joy 线程读到
    std::lock_guard<std::mutex> lk(binding_mutex_);
    binding_config_ = std::move(loaded);
    config_ = binding_config_.getTeleopConfig();
    return true;
  }

  /**
   * @brief 从 AMP 控制器配置合并 amp_hand 手柄参数
   */
  bool mergeAmpHandTeleopFromControllerConfig(
      const std::string& controller_config_path) {
    std::lock_guard<std::mutex> lk(binding_mutex_);
    return ParseAmpHandTeleopFromControllerConfig(controller_config_path,
                                                  config_);
  }
  ...
  void processInput(const InputData& data,
                    ...
                    double current_time) {
    // 整帧持锁: 与热重载 (setBindingConfig/loadBindingConfig) 互斥, 保护
    // config_ 与 binding_config_ 的读. 派生类的逐帧入口 (onJoyData/onQuestData)
    // 同样应在帧顶用 binding_mutex_ 上锁 (见各 adapter), 故下游 applyBindingForCombo /
    // applyDeadzone 不再自锁, 避免非递归 mutex 死锁.
    std::lock_guard<std::mutex> lk(binding_mutex_);
...
protected:
  ...
  /// 保护 config_ / binding_config_ 的读写: joy/quest 回调线程读 vs 热重载线程写.
  /// 派生类逐帧入口需用它上锁 (覆盖整帧), 写端 set/loadBindingConfig 已上锁.
