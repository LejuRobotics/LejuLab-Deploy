 ...
 */
struct ExternalHandTarget {
  std::vector<double> position;  ///< 左手6维 + 右手6维，范围 [0,100]

  bool isValid() const { return position.size() == 12; }
};

struct VersionedHandTarget {
  std::optional<ExternalHandTarget> value;
  uint64_t seq = 0;

  bool hasValue() const { return value.has_value(); }
  const ExternalHandTarget& getValue() const { return *value; }
...
struct MotionCommand {
  ...
  bool cmd_stance_valid = false;  ///< 是否携带 cmd_stance 覆盖
  int cmd_stance_mode = 0;        ///< 0=行走, 1=站立/下蹲/弯腰

  void setZero() {
    linear_x = linear_y = angular_z = 0.0;
    cmd_stance_valid = false;
    cmd_stance_mode = 0;
  }
