...
inline std::ostream& operator<<(std::ostream& os, LifecycleState state) {
    switch (state) {
        case LifecycleState::kWaitingForReady: os << "kWaitingForReady"; break;
        case LifecycleState::kWaitingForStart: os << "kWaitingForStart"; break;
        case LifecycleState::kRunning:          os << "kRunning"; break;
        case LifecycleState::kExiting:          os << "kExiting"; break;
        default:                                os << "Unknown(" << static_cast<int>(state) << ")"; break;
    }
    return os;
}
...
class Lifecycle {
 public:
  ...
  bool isRunning() const {
    return isNormalRunning();
  }

  bool isNormalRunning() const {
    return state_ == LifecycleState::kRunning;
  }

  bool allowsControlOutput() const {
    return state_ == LifecycleState::kRunning;
  }
