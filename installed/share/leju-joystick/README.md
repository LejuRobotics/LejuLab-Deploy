...
- runtime 尚未启动时按 `LB+RB+X`：启动 runtime，并在数据就绪后补发这次按下沿，直接进入 START 前倒地起身流程
...
- 按 `BACK`（BACK+START 组合）：**不再主动 `stopRuntime()`/`SendCtrl-C`**——由 runtime 的 ControlLoop 收到 `QuitSquat` 后先下蹲再自行退出；monitor 只登记优雅停止窗口，超时（`stop_grace`）仍强制清理 tmux session
