...
public:
    /**
     * @brief 读取IMU数据
     * @param imu_data 输出的IMU数据结构体
     * 
     * @return int32_t 返回0表示成功，非0表示失败
     *  -1: None IMU, 没有IMU
     *  -2: Read failed, 读取IMU数据失败
     */
    int32_t readImuData(leju::hw::ImuData_t &imu_data);
...
private:
    ...
    struct JointId2MotorMapping {
        struct MotorIdIndex_t {
            uint8_t id;
            uint8_t index;
        };

        using JointId_t = uint8_t;
        std::unordered_map<JointId_t, MotorIdIndex_t> jointId2data_;

        JointId2MotorMapping() { 
        }

        void add(uint8_t joint_id, uint8_t ruiwo_id, uint8_t index) {
            jointId2data_.insert(std::make_pair(joint_id, MotorIdIndex_t{ruiwo_id, index}));
        }

        /**
         * @brief 同类型驱动的 motor_id
         * 
         * @param joint_id 
         * @return uint8_t 
         */
        uint8_t motor_id(uint8_t joint_id) const {
            return jointId2data_.at(joint_id).id;
        }

        /**
         * @brief 同类型驱动的 index，0-based
         * 
         * @param joint_id 
         * @return uint8_t 
         */
        uint8_t motor_index(uint8_t joint_id) const {
            return jointId2data_.at(joint_id).index;
        }
    };

    JointId2MotorMapping jointId2RuiwoId_;
    ////////////////////////////////////////
